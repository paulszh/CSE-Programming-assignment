...
class DataFlowAnalysis {
  ...
  private:
		...
		void initializeBackwardMap(Function * func) {
			...
			for (Function::iterator bi = func->begin(), e = func->end(); bi != e; ++bi) {
				...
				// Initialize incoming edges to the basic block/Users/paulszh/Downloads/CSE231_Reference/Passes/DFA/ReachingDefinitionAnalysis.cpp
				for (auto pi = pred_begin(block), pe = pred_end(block); pi != pe; ++pi) {
					BasicBlock * prev = *pi;
					Instruction * src = (Instruction *)prev->getTerminator();
					Instruction * dst = firstInstr;
  ...
  public:
    ...
    void runWorklistAlgorithm(Function * func) {
        ...
        for (unsigned i = 1; i < IndexToInstr.size(); ++i){
        ...
        while (!worklist.empty()){
            ...
            Instruction * nodeInstr = IndexToInstr[node];
            getIncomingEdges(node, &IncomingEdges);
            getOutgoingEdges(node, &OutgoingEdges);
            
            flowfunction(nodeInstr, IncomingEdges, OutgoingEdges, Infos);  // Infos: info_out
            
            for (unsigned i = 0; i < OutgoingEdges.size(); ++i){
                Info *newInfo = new Info();
                Edge outEdge = std::make_pair(node, OutgoingEdges[i]);     // outEdge: outgoingedge[i]
                Info::join(EdgeToInfo[outEdge], Infos[i], newInfo);        // EdgeToInfo : m
