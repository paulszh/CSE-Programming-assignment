...
proc defineAttribute { } {
  ...
  define_user_attribute -type float -class cell CellSentivity
  return 1
}

proc PtCellSentivity { cell } {
	set sentivity 0.0
    set cellName [get_attri $cell base_name]
    ...
    if { $newCell != $libcellName } {
        ...
        set sentivity [expr $delta_leak * $old_slack / $pathNum]
    }

    return $sentivity
}

proc sortCellsSentivity { cell_collection } {
  
    foreach_in_collection cell $cell_collection {
      set_user_attribute $cell CellSentivity [PtCellSentivity $cell]
    }
    set new_collection [sort_collection -descending $cell_collection {CellSentivity}]
    return $new_collection
}
