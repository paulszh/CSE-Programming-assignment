...
proc defineAttributes { } {
  define_user_attribute -type float -class cell Sensitivity
  define_user_attribute -type string -class cell VtCell
  define_user_attribute -type string -class cell SizeCell
  define_user_attribute -type string -class cell SwapCell
  return 1
}

proc PtCellsensitivityVt { cell dicts } {
    set sensitivity 0.0
    set cellName [get_attri $cell base_name]
    ...
    #set old_leak [PtLeakPower]

    if { [dict exists $dicts $libcellName] } {
    	set old_leak [dict get $dicts $libcellName]
    } else {
        return $sensitivity
    }
    ...
    set newCell [getNextVtDown $libcellName]

    if { $newCell != $libcellName && $newCell != "skip" } {
        # size_cell $cellName $newCell
        # set new_leak [PtLeakPower]
        # size_cell $cellName $libcellName
        # puts "old $new_leak"

        if { [dict exists $dicts $newCell] } {
            set new_leak [dict get $dicts $newCell]
        } else {
            return $sensitivity
        }
        # puts "~~~~${newCell}~~~~"
        # puts "new $new_leak2"
        set delta_leak [expr $old_leak - $new_leak]
        set sensitivity [expr $delta_leak * $old_slack / $pathNum]
        set_user_attribute $cell VtCell $newCell
    }
    return $sensitivity
}

proc PtCellsensitivitySize { cell dicts } {
	set sensitivity 0.0
    set cellName [get_attri $cell base_name]
    set libcell [get_lib_cells -of_objects $cellName]
    set libcellName [get_attri $libcell base_name]
    ...
    set newCell [getNextSizeDown $libcellName]
    ...
    if { $newCell != $libcellName && $newCell != "skip" } {
        ...
        set delta_leak [expr $old_leak - $new_leak]
        set sensitivity [expr $delta_leak * $old_slack / $pathNum]
        set_user_attribute $cell SizeCell $newCell
    }
    return $sensitivity
}
...
proc check { cell } {
    ...
    foreach_in_collection inPin [get_pins -of $cell -filter "direction==in"] {
        set maxInTran [PtGetPinMaxTran $inPin]
        set currTran [PtGetPinTran $inPin]
        if { $maxInTran < $currTran } {
            return 0
        }
    }

    foreach_in_collection outPin [get_pins -of $cell -filter "direction==out"] {
        set maxCap [PtGetPinMaxCap $outPin]
        set currCap [PtGetPinLoadCap $outPin]
        set maxOutTran [PtGetPinMaxTran $outPin]
        set currTran [PtGetPinTran $outPin]
        if { $maxOutTran < $currTran ||  $maxCap < $currCap + 0.3 } {
            return 0
        }
    }
    return 1
...
proc getSensitivity { cell dicts } {
    set sensitivity1 [PtCellsensitivityVt $cell $dicts]
    set sensitivity2 [PtCellsensitivitySize $cell $dicts]
    if { $sensitivity1 > $sensitivity2} {
        set swap [get_attri $cell VtCell]
        set_user_attribute $cell Sensitivity $sensitivity1
        set_user_attribute $cell SwapCell $swap
    } elseif { $sensitivity2 > $sensitivity1 || $sensitivity2 != 0 } {
        set swap [get_attri $cell SizeCell]
        set_user_attribute $cell Sensitivity $sensitivity2
        set_user_attribute $cell SwapCell $swap
    } else {
...
proc sortCellssensitivity { cell_collection dicts } {
  
    foreach_in_collection cell $cell_collection {
        [getSensitivity $cell $dicts]
    }
